...
var getHAlarms = function(start, end, callback, error){
    ...
    $.ajax({
        url: $requestURI + 'gethistalarm?' + $systemAuth.token + '&' + start + '&' + end,
...
var setChart1 = function(data){
    ...
    var typeObject = _.groupBy(categories, function(item){ return item.DeviceType; });
...
var setChart3 = function(data){
    ...
    var timeObject = _.groupBy(data, function(item){ return moment(item.StartTime,'YYYY-MM-DD HH:mm:ss').format('MM/DD'); });
...
var setHistory = function(){
    ...
    getHAlarms(start, end, function(alarms){
