...
var setSize = function() {
    ...
    $('.py-fixed-tabs-bd').height(height - 135);
};
...
var setNet = function() {
    ...
    $.ajax({
        ...
        error: function(jqXHR, textStatus, errorThrown) {
            setMsg(msg, 'error', jqXHR.status + ':' + jqXHR.statusText + ' ' + jqXHR.responseText);
        }
    });
};
...
var setScrval = function() {
    ...
    setMsg(msg, 'loading', '正在加载...');
    $.ajax({
        url: $requestURI + 'getclientdefinedata?' + $systemAuth.token + '&pylon_screen_time',
        success: function(data, status) {
...
var savePwd = function() {
    ...
    $.ajax({
        url: $requestURI + 'setpd?' + $systemAuth.token + '&' + $.base64.btoa(newVal) + '&' + $.base64.btoa(oldVal),
        success: function(data, status) {
            if (isNullOrEmpty(data) === false) {
                if (data.startWith('Error') === false) {
                    if (data === 'true') {
...
var saveDate = function() {
    ...
    var datetime = moment(dateVal + ' ' + timeVal, 'YYYY-MM-DD HHmmss').format('YYYY-MM-DD HH:mm:ss');
    setMsg(msg, 'loading', '正在设置...');
    $.ajax({
...
var saveScrval = function() {
    ...
    setMsg(msg, 'loading', '正在设置...');
    $.ajax({
        url: $requestURI + 'setclientdefinedata?' + $systemAuth.token + '&pylon_screen_time',
